<script lang="ts">
    import {AddNewCustomItem, GetUserTableInfo, ScanCancel, ScanInHandler} from "../../wailsjs/go/main/App";
    ...
    let showNewItemPage: boolean = false;
    ...
    function onKeyDown(event: KeyboardEvent):void {
        if (currentView !== "Scan Listener" || showNewItemPage) {
            return
        }
        if (event.key === "Enter") {
            HandleScannedInItem()
    ...
    async function HandleScannedInItem(): Promise<void> {
        ...
        try {
            await ScanInHandler(itemPayload).then(r => {
                console.log(r)
                currentItemDetails = {
                    itemName: r.itemName,
    ...
    async function CancelScanIn(): Promise<void> {
        ...
        loading = true
        try {
            await ScanCancel(currentItemDetails.id)
            addToast({
    ...
    async function handleNewItemSubmit(event): Promise<void> {
        let itemDetails = event.detail
        const newItemDetails: database.CustomUPCModel = {
            itemName: itemDetails.itemName,
            imageUri: itemDetails.imageUri,
            upc: lastScannedUPC
        }
        try {
            await AddNewCustomItem(newItemDetails)
        } catch (e) {
            addToast({
                message:"Error adding new custom item: "+ e,
                type: "error",
                dismissible: true,
                timeout: 10000
            })
        ...
        addToast({
            message:"Successfully added new custom item",
            type: "success",
            dismissible: true,
            timeout: 3000
        })
        addToast({
            message:"Please rescan the same item to add it to the users inventory",
            type: "info",
            dismissible: true,
            timeout: 10000
        })
...
{/if}
{#if showNewItemPage}
    <AddCustomItem bind:isOpen={showNewItemPage} on:submit={handleNewItemSubmit}/>
{/if}
